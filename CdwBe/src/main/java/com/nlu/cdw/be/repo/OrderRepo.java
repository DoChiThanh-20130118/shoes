...
public interface OrderRepo extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT new com.nlu.cdw.be.domain.model.AmountYear(YEAR(o.createDate), COUNT(o.id), SUM (o.total)) FROM Order o WHERE o.orderStatus.id = 4 GROUP BY YEAR(o.createDate) ORDER BY SUM (o.total) DESC")
    List<AmountYear> reportAmountYear();
    @Query("SELECT new com.nlu.cdw.be.domain.model.AmountMonth(MONTH(o.createDate), COUNT(o.id), SUM (o.total)) FROM Order o WHERE o.orderStatus.id = 4 AND YEAR(o.createDate) = :year GROUP BY MONTH(o.createDate) ORDER BY SUM (o.total) DESC")
    List<AmountMonth> reportAmountMonth(@Param("year") Integer year);
    @Query("SELECT new com.nlu.cdw.be.domain.model.CountOrder(s.name, count(o.id)) FROM Order o INNER JOIN OrderStatus s on o.orderStatus.id = s.id GROUP BY s.name")
